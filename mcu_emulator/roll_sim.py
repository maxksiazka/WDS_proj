...
def create_sensor_packet():
    """Create a mock sensor data packet with a wing rocking (roll) maneuver"""
    sigma_accel = 0.2
    sigma_gyro = 0.05
    ...
    phi = max_roll_rad * math.sin(2 * math.pi * t / period)

    roll_rate_x = max_roll_rad * (2 * math.pi / period) * math.cos(2 * math.pi * t / period)

    accel_x_g = 0.0
    accel_y_g = 9.81 * math.sin(phi)
    accel_z_g = 9.81 * math.cos(phi)
    ...
    gyro = (
        roll_rate_x + random.gauss(0, sigma_gyro),
